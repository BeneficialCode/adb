...
*/
PFN_INSTWINUSBINTERFACE InstantiateWinUsbInterface = nullptr;
...
ADBAPIHANDLE __cdecl AdbEnumInterfaces(GUID class_id,
    ...
    bool active_only) {
    AdbInterfaceEnumObject* enum_obj = nullptr;
...
bool __cdecl AdbNextInterface(ADBAPIHANDLE adb_handle,
    ...
    if (nullptr == adb_ienum_object)
        return false;

    // Everything is verified. Pass it down to the object
    bool ret = adb_ienum_object->Next(pInfo, size);
...
ADBAPIHANDLE __cdecl AdbCreateInterfaceByName(
    ...
    AdbInterfaceObject* obj = nullptr;
    ADBAPIHANDLE ret = nullptr;
    ...
    try {
        ...
        if (IsLegacyInterface(interface_name)) {
            ...
            // been cached.
            if (nullptr != InstantiateWinUsbInterface) {
...
ADBAPIHANDLE __cdecl AdbCreateInterface(GUID class_id,
    ...
    if (0xFF == interface_id) {
        ...
        swprintf_s(match_name, L"\\\\?\\usb#vid_%04x&pid_%04x#",
            vendor_id, product_id);
    }
    else {
        // With interface id for the name.
        swprintf_s(match_name, L"\\\\?\\usb#vid_%04x&pid_%04x&mi_%02x#",
