...
#define SYS_PACKAGE_ID      0x01
    ...
    status_t parseIdmap(const void* idmap, size_t size, uint8_t* outPackageId, std::map<uint8_t, IdmapEntries>* outMap) {
        ...
        while (size > sizeof(uint16_t) * 4) {
            ...
            // outMap[entries.overlayTypeId()] = entries;
            auto iter = outMap->find(entries.overlayTypeId());
            if(iter ==outMap->end()) {
				outMap->insert(std::make_pair(entries.overlayTypeId(), entries));
			} else {
				return UNKNOWN_ERROR;
			}
            /*  size_t index = outMap->add(entries.overlayTypeId(), entries);
            if (index < 0) {
                return NO_MEMORY;
            }*/
            ...
            if (off < (mStringPoolSize - 1)) {
                if (!isUTF8) {
                    ...
                    if ((uint32_t)(u8str + *u8len - strings) < mStringPoolSize) {
                        ...
                        size_t actualLen = utf8_to_utf16_length(
        ...
        size_t strLen) const
    {
        ...
        if ((mHeader->flags & ResStringPool_header::UTF8_FLAG) != 0) {
            ...
            if (mHeader->flags & ResStringPool_header::SORTED_FLAG) {
                ...
                size_t l = 0;
                size_t h = mHeader->stringCount - 1;
    ...
    size_t ResXMLParser::getTextValue(Res_value* outValue) const
    {
        if (mEventCode == TEXT) {
    ...
    size_t ResXMLParser::getAttributeValue(size_t idx, Res_value* outValue) const
    {
        if (mEventCode == START_TAG) {
            const ResXMLTree_attrExt* tag = (const ResXMLTree_attrExt*)mCurExt;
            if (idx < dtohs(tag->attributeCount)) {
    ...
    size_t ResXMLParser::indexOfAttribute(const char16_t* ns, size_t nsLen,
    ...
    size_t ResXMLParser::indexOfID() const
    {
        if (mEventCode == START_TAG) {
            const size_t idx = dtohs(((const ResXMLTree_attrExt*)mCurExt)->idIndex);
            if (idx > 0) return (idx - 1);
        }
        return NAME_NOT_FOUND;
    }

    size_t ResXMLParser::indexOfClass() const
    {
        if (mEventCode == START_TAG) {
            const size_t idx = dtohs(((const ResXMLTree_attrExt*)mCurExt)->classIndex);
            if (idx > 0) return (idx - 1);
        }
        return NAME_NOT_FOUND;
    }

    size_t ResXMLParser::indexOfStyle() const
    {
        if (mEventCode == START_TAG) {
            const size_t idx = dtohs(((const ResXMLTree_attrExt*)mCurExt)->styleIndex);
            if (idx > 0) return (idx - 1);
        }
        return NAME_NOT_FOUND;
    }
    ...
    struct ResTable::PackageGroup
    {
        ...
        size_t findType16(const char16_t* type, size_t len) const {
            const size_t N = packages.size();
            for (size_t i = 0; i < N; i++) {
    ...
    status_t ResTable::Theme::applyStyle(uint32_t resID, bool force)
    {
        ...
        mTable.lock();
        const size_t N = mTable.getBagLocked(resID, &bag, &bagTypeSpecFlags);
        if (kDebugTableNoisy) {
        ...
        while (bag < end) {
            ...
            if (curPackage != p) {
                const size_t pidx = mTable.getResourcePackageIndex(attrRes);
                if (pidx < 0) {
            ...
            if (curType != t) {
                ...
                if (curEntries == NULL) {
                    ...
                    size_t cnt = typeList.empty() ? 0 : typeList[0]->entryCount;
                    size_t cnt_max = SIZE_MAX / sizeof(theme_entry);
    ...
    size_t ResTable::Theme::getAttribute(uint32_t resID, Res_value* outValue,
        uint32_t* outTypeSpecFlags) const
    {
        ...
        do {
            const size_t p = mTable.getResourcePackageIndex(resID);
    ...
    size_t ResTable::Theme::resolveAttributeReference(Res_value* inOutValue,
        size_t blockIndex, uint32_t* outLastRef,
    ...
    inline size_t ResTable::getResourcePackageIndex(uint32_t resID) const
    {
        return ((size_t)mPackageMap[Res_GETPACKAGE(resID) + 1]) - 1;
    }

    inline size_t ResTable::getResourcePackageIndexFromPackage(uint8_t packageID) const
    {
        return ((size_t)mPackageMap[packageID]) - 1;
    }
    ...
    status_t ResTable::add(ResTable* src, bool isSystemAsset)
    {
        ...
        for (size_t i = 0; i < src->mHeaders.size(); i++) {
            mHeaders.emplace_back(src->mHeaders[i]);
        }
        ...
        for (size_t i = 0; i < src->mPackageGroups.size(); i++) {
            ...
            for (size_t j = 0; j < srcPg->packages.size(); j++) {
                pg->packages.emplace_back(srcPg->packages[j]);
            }

            for (size_t j = 0; j < srcPg->types.size(); j++) {
                if (srcPg->types[j].empty()) {
                    continue;
                }
            ...
            mPackageGroups.emplace_back(pg);
        }
    ...
    bool ResTable::getResourceName(uint32_t resID, bool allowUtf8, resource_name* outName) const
    {
        ...
        const size_t p = getResourcePackageIndex(resID);
    ...
    size_t ResTable::getResource(uint32_t resID, Res_value* outValue, bool mayBeBag, uint16_t density,
    ...
    size_t ResTable::resolveReference(Res_value* value, size_t blockIndex,
        ...
        ResTable_config* outConfig) const
    {
        ...
        while (blockIndex >= 0 && value->dataType == Res_value::TYPE_REFERENCE
            ...
            uint32_t newFlags = 0;
            const size_t newIndex = getResource(value->data, value, true, 0, &newFlags,
                outConfig);
    ...
    size_t ResTable::lockBag(uint32_t resID, const bag_entry** outBag) const
    {
        mLock.lock();
        size_t err = getBagLocked(resID, outBag);
        if (err < NO_ERROR) {
    ...
    size_t ResTable::getBagLocked(uint32_t resID, const bag_entry** outBag,
        uint32_t* outTypeSpecFlags) const
    {
        if (mError != NO_ERROR) {
            return mError;
        ...
        if (parent) {
            ...
            const size_t NP = getBagLocked(resolvedParent, &parentBag, &parentTypeSpecFlags);
    ...
    void ResTable::setParameters(const ResTable_config* params)
    {
        ...
        std::lock_guard<std::mutex> _lock2(mFilteredConfigLock);
        ...
        for (size_t p = 0; p < mPackageGroups.size(); p++) {
            PackageGroup* packageGroup = mPackageGroups.at(p);
            if (kDebugTableNoisy) {
            ...
            for (size_t t = 0; t < packageGroup->types.size(); t++) {
                ...
                for (size_t ts = 0; ts < typeList.size(); ts++) {
                    Type* type = typeList.at(ts);

                    std::shared_ptr<std::vector<const ResTable_type*>> newFilteredConfigs =
                        std::make_shared<std::vector<const ResTable_type*>>();
                    ...
                    for (size_t ti = 0; ti < type->configs.size(); ti++) {
                        ...
                        if (config.match(mParams)) {
                            newFilteredConfigs->emplace_back(type->configs[ti]);
                        }
                    }
                    ...
                    cacheEntry.filteredConfigs.emplace_back(newFilteredConfigs);
                }
            }
        }
    }
    ...
    uint32_t ResTable::findEntry(const PackageGroup* group, size_t typeIndex, const char16_t* name,
        ...
        bool enforcePrivate) const
    {
        ...
        if (attrID != 0 && !Res_INTERNALID(attrID)) {
            const size_t p = getResourcePackageIndex(attrID);
            const bag_entry* bag = nullptr;
            size_t cnt = p >= 0 ? lockBag(attrID, &bag) : -1;
    ...
    void ResTable::getConfigurations(std::vector<ResTable_config>* configs, bool ignoreMipmap,
    ...
    bool ResTable::isPackageDynamic(uint8_t packageID) const {
        ...
        const size_t p = getResourcePackageIndexFromPackage(packageID);
        ...
        Entry* outEntry) const
    {
        ...
        for (size_t i = 0; i < typeCount; i++) {
            ...
            const std::vector<const ResTable_type*>* candidateConfigs = &typeSpec->configs;

            std::shared_ptr<std::vector<const ResTable_type*>> filteredConfigs;
            if (config && memcmp(&mParams, config, sizeof(mParams)) == 0) {
                // Grab the lock first so we can safely get the current filtered list.
                std::lock_guard<std::mutex> _lock(mFilteredConfigLock);
            ...
            for (size_t c = 0; c < numConfigs; c++) {
                const ResTable_type* const thisType = candidateConfigs->at(c);
                if (thisType == NULL) {
                    continue;
                }
        ...
        const Header* const header, bool appAsLib, bool isSystemAsset)
    {
        ...
        std::map<uint8_t, IdmapEntries> idmapEntries;
        ...
        if (idx == 0) {
            ...
            mPackageGroups.emplace_back(group);
            ...
            group = mPackageGroups.at(idx - 1);
            if (group == NULL) {
                return (mError = UNKNOWN_ERROR);
            }
        }

        group->packages.emplace_back(package);
        ...
        while (((const uint8_t*)chunk) <= (endPos - sizeof(ResChunk_header)) &&
            ...
            if (ctype == RES_TABLE_TYPE_SPEC_TYPE) {
                ...
                if ((dtohl(typeSpec->entryCount) > (INT32_MAX / sizeof(uint32_t))
                    || dtohs(typeSpec->header.headerSize) + (sizeof(uint32_t) * newEntryCount) > typeSpecSize)) {
                ...
                if (newEntryCount > 0) {
                    ...
                    auto iter = idmapEntries.find(typeSpec->id);
                    // size_t idmapIndex = idmapEntries.indexOfKey(typeSpec->id);
                    if (iter != idmapEntries.end()) {
                        typeIndex = iter->second.targetTypeId() - 1;
                    }
                ...
                if (newEntryCount > 0) {
                    ...
                    auto iter = idmapEntries.find(type->id);
                    // size_t idmapIndex = idmapEntries.indexOfKey(type->id);
                    if (iter != idmapEntries.end()) {
                        typeIndex = iter->second.targetTypeId() - 1;
                    }
                    else if (header->resourceIDMap != NULL) {
                    ...
                    if (addToType) {
                        ...
                        Type* t = typeList.at(typeList.size() - 1);
                        if (t->package != package) {
                            
                            return (mError = BAD_TYPE);
                        }

                        t->configs.emplace_back(type);
...
#define CHAR16_ARRAY_EQ(constant, var, len) \
        ...
        for (size_t pgIndex = 0; pgIndex < pgCount; pgIndex++) {
            ...
            for (size_t splitIndex = 0; splitIndex < splitCount; splitIndex++) {
                ...
                for (size_t typeIndex = 0; typeIndex < pg->types.size(); typeIndex++) {
                    ...
                    if (splitIndex >= typeList.size() || typeList.empty()) {
    ...
    status_t DynamicRefTable::addMappings(const DynamicRefTable& other)
    {
        ...
        for (const auto& entry : other.mEntries) {
            if (mEntries.find(entry.first) == mEntries.end()) {
                mEntries[entry.first] = entry.second;
            }
            else {
                if (entry.second != mEntries[entry.first]) {
                    return UNKNOWN_ERROR;
                }
            }
        }
    ...
    status_t DynamicRefTable::addMapping(const String16& packageName, uint8_t packageId)
    {
        ...
        size_t index = mEntries[packageName];
        if (index < 0) {
            return UNKNOWN_ERROR;
        }
