...
#define Res_MAKEARRAY(entry) (0x02000000 | (entry&0xFFFF))
    ...
    public:
        ...
        size_t getTextValue(Res_value* outValue) const;
        ...
        size_t getAttributeValue(size_t idx, Res_value* outValue) const;

        size_t indexOfAttribute(const char* ns, const char* attr) const;
        size_t indexOfAttribute(const char16_t* ns, size_t nsLen,
            const char16_t* attr, size_t attrLen) const;

        size_t indexOfID() const;
        size_t indexOfClass() const;
        size_t indexOfStyle() const;
 ...
 */
    ...
    public:
         ...
         */
        size_t getResource(uint32_t resID, Res_value* outValue, bool mayBeBag = false,
            uint16_t density = 0,
            uint32_t* outSpecFlags = NULL,
            ResTable_config* outConfig = NULL) const;

        inline size_t getResource(const ResTable_ref& res, Res_value* outValue,
            uint32_t* outSpecFlags = NULL) const {
            return getResource(res.ident, outValue, false, 0, outSpecFlags, NULL);
        }

        size_t resolveReference(Res_value* inOutValue,
            size_t blockIndex,
         ...
         */
        size_t lockBag(uint32_t resID, const bag_entry** outBag) const;

        void unlockBag(const bag_entry* bag) const;

        void lock() const;

        size_t getBagLocked(uint32_t resID, const bag_entry** outBag,
        ...
        public:
             ...
             * @return ssize_t Either a >= 0 table index or a negative error code.
             */
            size_t getAttribute(uint32_t resID, Res_value* outValue,
                uint32_t* outTypeSpecFlags = NULL) const;
             ...
             */
            size_t resolveAttributeReference(Res_value* inOutValue,
                size_t blockIndex, uint32_t* outLastRef = NULL,
                uint32_t* inoutTypeSpecFlags = NULL,
    ...
    private:
        ...
        struct TypeCacheEntry {
            ...
            // ResTable.
            std::vector<std::shared_ptr<std::vector<const ResTable_type*>>> filteredConfigs;
        };

        status_t addInternal(const void* data, size_t size, const void* idmapData, size_t idmapDataSize,
            bool appAsLib, const int32_t cookie, bool copyData, bool isSystemAsset = false);

        size_t getResourcePackageIndex(uint32_t resID) const;
        size_t getResourcePackageIndexFromPackage(uint8_t packageID) const;
        ...
        uint32_t findEntry(const PackageGroup* group, size_t typeIndex, const char16_t* name,
        ...
        mutable std::mutex               mFilteredConfigLock;
