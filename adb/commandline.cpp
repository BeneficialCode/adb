...
static int adb_shell(int argc, const char** argv) {
    ...
    int opt;
    while ((opt = getopt_a(argc, const_cast<char**>(argv), "+e:ntTx")) != -1) {
        switch (opt) {
        case 'e':
            if (!(strlen(optarg_a) == 1 || strcmp(optarg_a, "none") == 0)) {
                error_exit("-e requires a single-character argument or 'none'");
            }
            escape_char = (strcmp(optarg_a, "none") == 0) ? 0 : optarg_a[0];
            break;
        case 'n':
            close_stdin();
            break;
...
static bool adb_root(const char* command) {
    ...
    while (bytes_left > 0) {
        size_t bytes_read = adb_read(fd, cur, bytes_left);
        if (bytes_read == 0) {
            break;
        }
...
int adb_commandline(int argc, const char** argv) {
    ...
    /* do_sync_*() commands */
        ...
        int opt;
        while ((opt = getopt_a(argc, const_cast<char**>(argv), "lnz:Z")) != -1) {
            switch (opt) {
            case 'l':
                list_only = true;
                break;
            case 'n':
                ...
                break;
            case 'z':
                compression = parse_compression_type(optarg_a, false);
                break;
            case 'Z':
                compression = CompressionType::None;
                break;
