...
static bool should_pull_file(ULONG mode) {
    return S_ISREG(mode) || S_ISBLK(mode) || S_ISCHR(mode);
}

static bool should_push_file(ULONG mode) {
...
public:
    ...
    bool SendSend2(std::string_view path, ULONG mode, CompressionType compression, bool dry_run) {
        if (path.length() > 1024) {
            Error("SendRequest failed: path too long: %zu", path.length());
            errno = ENAMETOOLONG;
            return false;
        ...
        p = static_cast<char*>(memcpy(p, &msg.send_v2_setup, sizeof(msg.send_v2_setup))) + sizeof(msg.send_v2_setup);
...
public:
    ...
    // difference to "adb sync" performance.
    bool SendSmallFile(const std::string& path, ULONG mode, const std::string& lpath,
    ...
    bool SendLargeFile(const std::string& path, ULONG mode, const std::string& lpath,
        ...
        if (!SendSend2(path, mode, compression, dry_run)) {
            ...
            Error("failed to send ID_SEND_V2 message '%s': %s", path.c_str(), error);
            return false;
        }
        ...
        if (stat(lpath.c_str(), &st) == -1) {
            char error[256];
            strerror_s(error, sizeof(error), errno);
            Error("cannot stat '%s': %s", lpath.c_str(), error);
            return false;
        }
        ...
        if (lfd < 0) {
            char error[256];
            strerror_s(error, sizeof(error), errno);
            Error("opening '%s' locally failed: %s", lpath.c_str(), error);
            return false;
        }
        ...
        while (sending) {
            ...
            if (r < 0) {
                char error[256];
                strerror_s(error, sizeof(error), errno);
                Error("reading '%s' locally failed: %s", lpath.c_str(), error);
                return false;
            }
    ...
    bool SendLargeFileLegacy(const std::string& path, ULONG mode, const std::string& lpath,
    ...
    bool WriteOrDie(const std::string& from, const std::string& to, const void* data,
        ...
        if (!WriteFdExactly(fd, data, data_length)) {
            if (errno == ECONNRESET) {
                ...
                char errormsg[256];
                strerror_s(errormsg, sizeof(errormsg), errno);
                Error("%zu-byte write failed: %s", data_length, errormsg);
            }
            _exit(1);
        }
        return true;
...
static bool sync_send(SyncConnection& sc, const std::string& lpath, const std::string& rpath,
    unsigned mtime, ULONG mode, bool sync, CompressionType compression,
    bool dry_run) {
    if (sync) {
        struct stat st;
    ...
    if (stat(lpath.c_str(), &st) == -1) {
        char errormsg[256];
        strerror_s(errormsg, sizeof(errormsg), errno);
        sc.Error("failed to stat local file '%s': %s", lpath.c_str(), errormsg);
        return false;
    }
    if (st.st_size < SYNC_DATA_MAX) {
        ...
        if (!android::base::ReadFileToString(lpath, &data, true)) {
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Error("failed to read all of '%s': %s", lpath.c_str(), errormsg);
            return false;
        }
...
static bool sync_recv_v1(SyncConnection& sc, const char* rpath, const char* lpath, const char* name,
    ...
    if (lfd < 0) {
        char errormsg[256];
        strerror_s(errormsg, sizeof(errormsg), errno);
        sc.Error("cannot create '%s': %s", lpath, errormsg);
        return false;
    }
    ...
    while (true) {
        ...
        if (!WriteFdExactly(lfd, buffer, msg.data.size)) {
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Error("cannot write '%s': %s", lpath, errormsg);
            adb_unlink(lpath);
            return false;
        }
...
static bool local_build_list(SyncConnection& sc, std::vector<copyinfo>* file_list,
    ...
    if (!dir) {
        char errormsg[256];
        strerror_s(errormsg, sizeof(errormsg), errno);
        sc.Error("cannot open '%s': %s", lpath.c_str(), errormsg);
        return false;
    }
...
bool do_sync_push(const std::vector<const char*>& srcs, const char* dst, bool sync,
    ...
    if (sync_stat_fallback(sc, dst, &st)) {
        ...
        if (errno == ENOENT || errno == ENOPROTOOPT) {
            ...
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Error("stat failed when trying to push to %s: %s", dst, errormsg);
            return false;
        }
    }
    ...
    for (const char* src_path : srcs) {
        ...
        if (stat(src_path, &st) == -1) {
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Error("cannot stat '%s': %s", src_path, errormsg);
            success = false;
            continue;
        }
...
static bool remote_build_list(SyncConnection& sc, std::vector<copyinfo>* file_list,
    ...
    for (copyinfo& link_ci : linklist) {
        ...
        if (!sync_stat_fallback(sc, link_ci.rpath, &st)) {
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Warning("stat failed for path %s: %s", link_ci.rpath.c_str(), errormsg);
            continue;
        }
...
static int set_time_and_mode(const std::string& lpath, time_t time,
    ...
    int mask;
    int old_mask;
    _umask_s(0000, &old_mask);
    _umask_s(old_mask, &mask);
    ...
    if (stat(dst, &st) == -1) {
        ...
        if (srcs.size() == 1 && errno == ENOENT) {
            ...
            if (stat(android::base::Dirname(dst).c_str(), &parent_st) == -1) {
                char errormsg[256];
                strerror_s(errormsg, sizeof(errormsg), errno);
                sc.Error("cannot create file/directory '%s': %s", dst, errormsg);
                return false;
            }
        }
        else {
            char errormsg[256];
            strerror_s(errormsg, sizeof(errormsg), errno);
            sc.Error("failed to access '%s': %s", dst, errormsg);
            return false;
        }
    }
    ...
    for (const char* src_path : srcs) {
        ...
        if (!sync_stat_fallback(sc, src_path, &src_st)) {
            if (errno == ENOPROTOOPT) {
                ...
                char errormsg[256];
                strerror_s(errormsg, sizeof(errormsg), errno);
                sc.Error("failed to stat remote object '%s': %s", src_path, errormsg);
            }
