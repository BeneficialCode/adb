...
namespace incremental {
    ...
    static inline constexpr T toBigEndian(T t) {
        ...
        if constexpr (std::is_same_v<T, int16_t>) {
            return htons(static_cast<unsigned_type>(t));
        }
        else if constexpr (std::is_same_v<T, int32_t>) {
            return htonl(static_cast<unsigned_type>(t));
        }
        else if constexpr (std::is_same_v<T, int64_t>) {
            return htonll(static_cast<unsigned_type>(t));
        }
        else {
            return t;
        }
    ...
    static inline constexpr T readBigEndian(void* data) {
        using unsigned_type = std::make_unsigned_t<T>;
        if constexpr (std::is_same_v<T, int16_t>) {
            return static_cast<T>(ntohs(*reinterpret_cast<unsigned_type*>(data)));
        }
        else if constexpr (std::is_same_v<T, int32_t>) {
            return static_cast<T>(ntohl(*reinterpret_cast<unsigned_type*>(data)));
        }
        else if constexpr (std::is_same_v<T, int64_t>) {
            return static_cast<T>(ntohll(*reinterpret_cast<unsigned_type*>(data)));
        }
        else {
            return T();
        }
    ...
    bool IncrementalServer::SkipToRequest(void* buffer, size_t* size, bool blocking) {
        while (true) {
            ...
            for (bcur = 0; bcur + 4 < bsize; ++bcur) {
                uint32_t magic = ntohl(*(uint32_t*)(buffer_.data() + bcur));
                if (magic == INCR) {
                    magic_found = true;
                    break;
                }
