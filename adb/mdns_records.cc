    ...
    namespace discovery {
        ...
        namespace {
            ...
            bool IsGreaterThan(const Rdata& lhs, const Rdata& rhs) {
                ...
                auto lhs_bytes = std::make_unique<uint8_t[]>(lhs_size);
                auto rhs_bytes = std::make_unique<uint8_t[]>(rhs_size);
                MdnsWriter lhs_writer(lhs_bytes.get(), lhs_size);
                MdnsWriter rhs_writer(rhs_bytes.get(), rhs_size);
