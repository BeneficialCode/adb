...
size_t adb_sendmsg(borrowed_fd fd, const struct adb_msghdr* msg, int flags) {
    FH fh = _fh_from_int(fd, __func__);
...
size_t adb_recvmsg(borrowed_fd fd, struct adb_msghdr* msg, int flags) {
    FH fh = _fh_from_int(fd, __func__);
...
static int _network_server(int port, int type, u_long interface_address, std::string* error) {
    ...
    if (bind(s, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
        ...
        char ip[INET_ADDRSTRLEN] = "";
        InetNtopA(AF_INET, &addr.sin_addr, ip, sizeof(ip));
        *error = android::base::StringPrintf("cannot bind to %s:%u: %s", ip,
            ntohs(addr.sin_port),
            android::base::SystemErrorCodeToString(err).c_str());
...
int adb_socket_get_local_port(borrowed_fd fd) {
    ...
    if (adb_getsockname(fd, reinterpret_cast<sockaddr*>(&addr_storage), &addr_len) < 0) {
        char error_msg[256] = "";
        strerror_s(error_msg, sizeof(error_msg), errno);
        D("adb_socket_get_local_port: adb_getsockname failed: %s", error_msg);
        return -1;
    }
...
int adb_socketpair(int sv[2]) {
    ...
    if (accepted < 0) {
        char error_msg[256] = "";
        strerror_s(error_msg, sizeof(error_msg), errno);
        D("adb_socketpair: failed to accept: %s", error_msg);
        goto fail;
    }
    adb_close(server);
    sv[0] = client;
...
HANDLE _get_console_handle(borrowed_fd fd, DWORD* mode) {
...
static inline bool _is_shift_pressed(const DWORD control_key_state) {
    return (control_key_state & SHIFT_PRESSED) != 0;
}

static inline bool _is_ctrl_pressed(const DWORD control_key_state) {
    return (control_key_state & (LEFT_CTRL_PRESSED | RIGHT_CTRL_PRESSED)) != 0;
}

static inline bool _is_alt_pressed(const DWORD control_key_state) {
    return (control_key_state & (LEFT_ALT_PRESSED | RIGHT_ALT_PRESSED)) != 0;
}

static inline bool _is_numlock_on(const DWORD control_key_state) {
    return (control_key_state & NUMLOCK_ON) != 0;
}

static inline bool _is_capslock_on(const DWORD control_key_state) {
    return (control_key_state & CAPSLOCK_ON) != 0;
}

static inline bool _is_enhanced_key(const DWORD control_key_state) {
...
static inline size_t _get_non_control_char(char* const ch,
    const KEY_EVENT_RECORD* const key_event, const DWORD control_key_state) {
...
// Get without Alt.
static inline size_t _get_non_alt_char(char* const ch,
    const KEY_EVENT_RECORD* const key_event, const DWORD control_key_state) {
    return _get_char_ignoring_modifier(ch, key_event, control_key_state,
        VK_MENU);
}
...
NarrowArgs::NarrowArgs(const int argc, wchar_t** const argv) {
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        narrow_args[i] = _strdup(arg_narrow.c_str());
    }
...
int unix_open(std::string_view path, int options, ...) {
    ...
    if ((options & O_CREAT) == 0) {
        int fd = 0;
        errno_t err = _wsopen_s(&fd, path_wide.c_str(), options, _SH_DENYNO, 0);
        return fd;
    }
    else {
        int mode;
...
struct dirent* adb_readdir(DIR* dir) {
    ...
    strcpy_s(ent->d_name, name_utf8.c_str());
...
FILE* adb_fopen(const char* path, const char* mode) {
    ...
    FILE* fp = nullptr;
    _wfopen_s(&fp, path_wide.c_str(), mode_wide.c_str());
    return fp;
}
...
char* adb_getcwd(char* buf, int size) {
    ...
    strcpy_s(buf, size, buf_utf8.c_str());
...
int adb_thread_setname(const std::string& name) {
    ...
    auto set_thread_description_func = reinterpret_cast<PSetThreadDescription>(
