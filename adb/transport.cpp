...
namespace {
    ...
    private:
        ...
        _Guarded_by_(reconnect_mutex_) bool running_ = true;
        std::thread handler_thread_;
        std::condition_variable reconnect_cv_;
        _Guarded_by_(reconnect_mutex_) std::set<ReconnectAttempt> reconnect_queue_;
...
static int transport_read_action(int fd, struct tmsg* m) {
    ...
    while (len > 0) {
        ...
        if (r > 0) {
            ...
            D("transport_read_action: on fd %d: %s", fd, error);
            return -1;
        }
    }
    return 0;
...
static int transport_write_action(int fd, struct tmsg* m) {
    ...
    while (len > 0) {
        ...
        if (r > 0) {
            ...
            char error[256];
            strerror_s(error, sizeof(error), errno);
            D("transport_write_action: on fd %d: %s", fd, error);
            return -1;
        }
    }
    return 0;
