...
private:
    ...
    _Guarded_by_(mutex_) bool started_ = false;
    _Guarded_by_(mutex_) bool stopped_ = false;

    std::unique_ptr<BlockingConnection> underlying_;
    _Guarded_by_(mutex_) std::thread read_thread_;
    _Guarded_by_(mutex_) std::thread write_thread_;

    _Guarded_by_(mutex_) std::deque<std::unique_ptr<apacket>> write_queue_;
    std::mutex mutex_;
...
private:
    _Guarded_by_(mutex_) bool connection_established_ = false;
    _Guarded_by_(mutex_) bool connection_established_ready_ = false;
    std::mutex mutex_;
    std::condition_variable cv_;
...
private:
    ...
    _Guarded_by_(mutex_) std::shared_ptr<Connection> connection_;
