...
struct LibusbConnection : public Connection {
    ...
    void Cleanup(ReadBlock* read_block){
    ...
    bool MaybeCleanup(ReadBlock* read_block){
        CHECK(read_block);
    ...
    void CancelReadTransfer(ReadBlock* read_block){
    ...
    ReadBlock header_read_;
    ReadBlock payload_read_;
    std::optional<amessage> incoming_header_;
    IOVector incoming_payload_;

    std::mutex write_mutex_;
    std::unordered_map<TransferId, std::unique_ptr<WriteBlock>> writes_;
...
static std::mutex usb_handles_mutex;
...
static int WINAPI hotplug_callback(libusb_context*, libusb_device* device,
