...
int known_device_locked(const wchar_t* dev_name) {
    if (nullptr != dev_name) {
        ...
        for (usb_handle* usb : handle_list) {
            ...
            if ((nullptr != usb->interface_name) && (0 == _wcsicmp(usb->interface_name, dev_name))) {
                return 1;
            }
        }
    }
...
usb_handle* do_usb_open(const wchar_t* interface_name) {
    ...
    if (nullptr == ret) {
        ...
        D("Could not allocate %u bytes for usb_handle: %s", sizeof(usb_handle), error);
        goto fail;
    }
    ...
    if (nullptr == ret->interface_name) {
        char error[256];
        strerror_s(error, sizeof(error), errno);
        D("Could not allocate %lu characters for interface_name: %s", name_len, error);
        goto fail;
    }
...
void find_devices() {
    ...
    AdbInterfaceInfo* next_interface = nullptr;
    unsigned long entry_buffer_size = sizeof(next_interface);
