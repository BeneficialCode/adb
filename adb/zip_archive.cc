...
#else
        _close(mapped_zip.GetFileDescriptor());
#endif
    }
}
...
static int32_t FindEntry(const ZipArchive* archive, std::string_view entryName,
    ...
    local_header_end = local_header_offset + sizeof(LocalFileHeader);
    ...
    off64_t name_offset;
    name_offset = local_header_offset + sizeof(LocalFileHeader);

    off64_t name_end;
    name_end = name_offset + name_length;
    if(name_end > cd_offset) {
        return kInvalidOffset;
	}
...
public:
    ...
    static std::optional<FileWriter> Create(int fd, const ZipEntry64* entry) {
        const uint64_t declared_length = entry->uncompressed_length;
        const off64_t current_offset = _lseeki64(fd, 0, SEEK_CUR);
        if (current_offset == -1) {
            return {};
        }
        ...
        if (!S_ISBLK(sb.st_mode)) {
            long result = TEMP_FAILURE_RETRY(_chsize_s,fd, declared_length + current_offset);
            if (result == -1) {
...
void zstream_deleter(z_stream* stream) {
...
const uint8_t* MappedZipFile::ReadAtOffset(uint8_t* buf, size_t len, off64_t off) const {
    ...
    if (data_length_ != -1) {
        ...
        read_end = off + static_cast<off64_t>(len);
