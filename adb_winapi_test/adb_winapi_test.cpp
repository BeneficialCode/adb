// adb_winapi_test.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
#include <Windows.h>
#include <atlbase.h>
#include <algorithm>
...
bool TestEnumInterfaces() {
    ...
    AdbInterfaceInfo* interface_info = nullptr;
    ULONG buf_size = sizeof(interface_info);
...
bool TestInterfaces() {
    ...
    if (NULL == enum_handle) {
        ...
        AdbInterfaceInfo* interface_info = nullptr;
        unsigned long buf_size = sizeof(AdbInterfaceInfo);
...
bool TestInterfaceName(ADBAPIHANDLE interface_handle) {
    ...
    buf_size = intr_name_size + 16; // extra in case of overwrite
...
std::string get_error_description(const DWORD err) {
    ...
    if (found != end) {
        ...
        char buf[64];
        _snprintf_s(buf, sizeof(buf), "%u", err);
